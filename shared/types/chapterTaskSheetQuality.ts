...
export function stripNegatedOpponentDeclarations(
  ...
  let out = text;
  // 1) 【禁止】/【禁止 X】整段：从「【禁止」标记扣到下一个「下一个【X】段或文末
  //    （覆盖最常见 markdown 化的禁项段；段后常接下一个【X】段或文末）
  out = out.replace(/【禁止】[^【]*?(?=【|$)/g, "");
  out = out.replace(/【禁止[^\]】\s]*】[^【]*?(?=【|$)/g, "");
  // 1b) 风险/警示/避免类分区头整段（「风险提醒:」「【风险提示】」「风险:」等）。
  //     这些分区整段都是「告诫写作不可怎样」的避免声明，正文里不含「实际写成的内容」，
  //     复述禁词属于正确声明禁项。剥到下一个分区头或文末。
  //     注意: 只剥「风险提示」类头, 不碰【必须推进】【本章独占事件】等正分区。
  out = out.replace(/(?:【风险提示】|【风险提醒】|风险提示[:：]|风险提醒[:：])[^【]*?(?=【|$)/g, "");
  out = out.replace(/^(?:风险提醒|风险提示)：[^\n]*(?:\n(?:[-*]?\s+[^\n]*\n|[-*]?\s+[^\n]*$))*$/m, "");
