...
- 整本章节流水线、自动导演和全书音频封装默认采用保守的单任务执行，并允许在确认宿主资源充足后受控调高，降低连续重启和间歇性不可用风险。
- 有声书章节重做、续生成和后台封装会识别当前任务代次；较早的进程不会覆盖新音频，封装结果暂时写入失败时也会继续补写状态。
- 请求内容不是合法 JSON 时会明确返回参数格式错误，不再显示为服务器内部故障。

### 2026-09-04

- 有声书全书 m4b 封装在服务重启后会自动重新接管并完成封装，不再因为后台任务与页面任务分离而长期停留在“封装中”。
...
- 自动写章和后台修文会持续核对正文版本；当你在生成、审校、修复或状态回写期间手动修改章节时，较早启动的结果不会覆盖新正文，也不会把旧评分或“已完成”状态附着到新内容上。
- 章节重写后，后续写作只会读取当前正文对应的自动事实；旧版本事实会退出连续性上下文，手动补充的事实仍会保留，减少后文引用已经被改掉的剧情。
- 章节正文更新后，旧正文抽取出的自动时间线事件、钩子和时间锚点不会再进入后续写作上下文；手动维护的时间线内容会继续保留。
- 章节正文更新后，较早启动的摘要、状态、角色动态、资源风险和伏笔整理会停止写入；后续检索内容只会在当前章节资产完整同步后刷新，减少改稿后仍引用旧剧情的情况。
- 任务重启恢复不会抢占仍由有效执行者处理的取消任务；只有无人持有或租约已经过期的任务才由恢复流程收口，降低重复执行和状态跳变风险。
- 在任务恢复或更换后台执行者的同时发出取消，取消请求仍会作用于当前任务，不会因为执行者刚好切换而继续运行。
- 从任务链接打开已取消的自动导演流程时，任务详情会保持可见，方便回看取消位置、失败原因和已经生成的结果。
- 卷级章节复检会以实际审过的正文版本收口状态；正文并发变化时会保留待复检，而不是把旧审校结果当作新正文已经通过。

### 2026-07-27

- 后台章节修文会在落稿前确认正文版本；如果你在生成期间手动改过正文，系统会保留你的编辑，不会让较早启动的修文结果覆盖它。
- 自动成书遇到可继续的局部质量问题时，会记录待优化项并继续完成后续章节；只有明确需要重规划、正文不可用或运行状态无法安全推进时才会暂停。
- 章节任务的取消、超时、重试和恢复状态更加稳定；任务恰好在取消时完成，也会保留真实完成结果，不会把关联的自动导演任务误标为已取消。失败、阻塞或等待恢复的自动导演任务也能持续出现在任务入口中，不会被普通工作区任务误替代。
- 新部署会完整建立章节连续性与任务恢复所需的数据结构，章节上下文与重启恢复不再因结构缺失而降级。
