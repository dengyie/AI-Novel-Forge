...
- `prose_*` 问题默认属于本章局部质量问题。自动修复耗尽后，若**无** non-deferrable L0（sot/HUD/critical prose）且正文仍可读，可登记 `defer_and_continue` 为 non_blocking 质量债并继续剩余章节；**有** non-deferrable L0 时 defer 仍 blocking（`l0Clear=false`）。不得仅因为单章可 defer 的自然度提示写入 `replanAlertDetails`、`PIPELINE_REPLAN_REQUIRED` 或全局自动导演重规划。
