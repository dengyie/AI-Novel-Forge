...
- **pipeline 取消必须跟随 canonical job，而不是跟随请求时看到的旧租约**：排队任务只能用 `status=queued` CAS 进入取消终态；运行任务用 `status=running + finishedAt=null` CAS 写入取消请求，即使 worker 在请求窗口内发生租约接管，取消仍应作用于同一任务。CAS 未命中必须重读 canonical job 并返回数据库现状，禁止用无条件 update 覆盖并发成功、失败或重排队。运行 worker 如果因取消导致心跳 CAS 未命中，即使错误表象是 `PIPELINE_LEASE_LOST`，恢复层也必须先尝试以 worker 持有的 owner 收口 canonical cancellation；只有取消状态不存在或 owner CAS 未命中时，才按真正租约转移静默退出。取消关联导演任务必须以重读后的 canonical pipeline 状态为准：只有结果确实为 `cancelled` 才能传播取消。等待运行器收口的取消行保持 `finishedAt=null`，收口时必须清理 `cancelRequestedAt`、租约和心跳并写入完成时间。
...
- **pipeline 高负载执行必须经过进程级准入**：`startPipelineJob`、启动恢复、stale watchdog 与自动重排队虽然入口不同，都必须在数据库租约认领前汇入同一个执行许可池。默认一次只执行一个完整 Pipeline，`PIPELINE_EXECUTION_CONCURRENCY` 可显式调高但硬上限为 4；排队只占内存调度标记，不得提前持有会过期的数据库租约。
...
- **修文 adopt / discard（写文质量 P0）**：自动修文路径（`ChapterRepairStreamRuntime.finalizeRepairResult`）必须 `baseline → candidate → evaluate → adopt|discard`，**禁止**未评估先写 `chapter.content`。baseline 与 candidate **同协议 evaluateOnly**（不优先信旧 QualityReport）。降 overall、新增 L0 hard、新增 L1 义务/审校硬伤、或文学门恶化 → discard（正文保持 baseline，repairHistory 记 decision）；改进且无 L0/L1 恶化 → adopt。连续无改进达 plateau → 停自动修、记债，**禁止**策略化 `skip_quality_repair`。
- **文学 isPass 与质量过审**：`isPass` / `isLiteraryQualityPass` 固定 `coherence≥80 ∧ repetition≥75 ∧ engagement≥75`。qualityLoop `literary_score` / retention 与 isPass 同阈值。`!literaryPass` 不得写成 `chapterStatus=completed` 的质量过审（manual review / repair recheck / pipeline auto-review）；`mergeChapterPatchForGenerationStateBump` 无 `literaryPass:true` 证明不得 completed；导演 `defer_and_continue` 可读可续，但不是质量过审。L0 另含 `sot_banned_term` / `sot_must_avoid_leak`（高严重度；书级词表键 `sotBannedTerms` 于 storyWorldSlice(Overrides)Json，默认空）。
- **L0 硬门（写文质量硬门）**：政策禁词 / critical prose / 结构 HUD（`prose_system_hud`，扫原文 `【…】`）有码即抬 `prose_quality` **invalid**；`hasNonDeferrableProseOrSotDebt` + `classifyChapterQualityLoopRisk` 使 `terminalAction=defer_and_continue` **仍 blocking**，不得降 non_blocking。空 `sotBannedTerms` **不**假拦（book-agnostic）。DTO：`literaryPass`（文学三维）与 **`l0Clear`**（无 non-deferrable L0）正交。短章 `length_under_*` 只进 `observabilityTags`，不进 isPass。权威：`docs/plans/writing-quality-hardgate-architecture-plan.md`。
- **指代 AI 味 L0（pronoun）**：确定性检测 `prose_pronoun_subject_stack`（连续句首他/她 run hard=4 / critical=6）与 `prose_pronoun_density`（hard=0.45，soft=`prose_pronoun_density_soft`=0.35，最少 8 句）。仅 hard 码入 `NON_DEFERRABLE_PROSE_OR_SOT_ISSUE_CODES` 并抬 `prose_quality` invalid；soft 只可观测。开篇 `order≤3` 的 writer `style_contract` 追加固定声线「开篇忌连续句首他/她；优先专名与动作起句」（`buildWriterStyleContractText(..., { chapterOrder })`），不改 StyleContract schema。
- **PostGeneration 热点段落改写**：`PostGenerationStyleReviewRunner` 在整章风格轮之外，对 pronoun 热点段（`HotspotParagraphRewrite`：minRun=3、maxHotspots=4、K=2 候选）做 multi-candidate evaluate-adopt。引入 HUD / sot / critical 非 pronoun 硬回归或字数塌缩的候选丢弃；无改善保持 baseline。整 runner 抛错仍可回退原文，但已检出的 pronoun hard L0 必须能挡质量过审。
- **styleClear 双门**：`completed` 仅当 `literaryPass ∧ styleClear`。`projectStyleClear`：blocking pronoun 永远 false；开篇 `order≤DEFAULT_STYLE_GATE_MAX_ORDER(3)` 时 residual≥35 或 residual 未知（null）→ false；中盘仅 residual 高 → styleClear true + qualityLoop `style_residual` risk 记债 + `continue`（non_blocking）。qualityLoop 新增 `style_pronoun` / `style_residual` signal；repair 无完整 residual 时开篇保守挡 completed。**禁止**策略化 `skip_quality_repair` / `writingQualityMode` 主路径。权威：`docs/plans/2026-07-17-prose-quality-ai-taste-revision-plan.md`。
- `prose_*` 问题默认属于本章局部质量问题。自动修复耗尽后，若**无** non-deferrable L0（sot/HUD/critical prose/pronoun hard）且正文仍可读，可登记 `defer_and_continue` 为 non_blocking 质量债并继续剩余章节；**有** non-deferrable L0 时 defer 仍 blocking（`l0Clear=false`）。不得仅因为单章可 defer 的自然度提示写入 `replanAlertDetails`、`PIPELINE_REPLAN_REQUIRED` 或全局自动导演重规划。
...
- 第 3-8 章这类章节都显示“建议补写修复 / 质量需修复”：先检查 `riskFlags.qualityLoop`。若是 `defer_and_continue` **且**仅 L2 literary / 可 defer 的 medium-low prose（`l0Clear=true`，无 sot/HUD/critical），主界面和 AI 驾驶舱不得把它显示为阻塞错误。若含 non-deferrable L0（`sot_*` / `prose_system_hud` / critical prose），classify 为 **blocking**（`l0Clear=false`），应显示为需修硬伤，不得当可读债忽略。
