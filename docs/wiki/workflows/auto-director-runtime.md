...
- 章节批次自动执行必须建立 run-local ownership fence。Command lease 的中止信号、任务 `cancelRequestedAt / cancelled` 状态和当前 pipeline job id 共同决定该执行是否仍可写入；`syncAutoExecutionTaskState`、workflow checkpoint、质量债与 repair ticket、重规划调用、批续窗章节准备、异步提案确认及 task terminal 投影都必须在副作用前通过同一个 fence。明确取消或 AbortSignal aborted 时可以取消该旧执行跟踪的 pipeline job 并直接退出；最终 workflow 写入发生 CAS miss 时只能判定 ownership lost，不得再取消 job、清理 runtime 或回写旧 seed。任何 ownership lost 路径都禁止用 best-effort cleanup 把旧 `autoExecution` 状态写回新 retry。
- ownership fence 的任务事实快照必须包含 `attemptCount + ownershipVersion`，但这两个字段不能在首次读取时被旧 Worker 无条件认领。Command Worker 路径还必须绑定租约时已经确定的 `commandId + leaseOwner + leaseAttempt`：首次 task 读取前先校验 command raw lease，最终 workflow/proposal 事务再校验 command 仍属于该执行（`leased/running`、owner/attempt 一致、lease 未过期）。`ownershipVersion` 是专用的执行所有权版本：workflow 状态投影、command acceptance、取消、重试、恢复和 fenced terminal write 会递增它；LLM token 聚合、自动审批里程碑等不改变执行权的遥测或审计元数据不得改变它。最终 workflow `updateMany` 必须同时校验 task id、`lane=auto_director`、允许的活动状态、`cancelRequestedAt=null`、attempt 和 ownershipVersion；跨小说提交 proposal/canonical state 时还必须校验 workflow task 的 `novelId`。任一 command/task CAS miss 统一转换为 ownership lost。
- task CAS 提交是明确的 ownership handoff 边界。数据库返回的新 `attemptCount + ownershipVersion` 必须在 transition notification、canonical snapshot/version、automation ledger 等后置副作用开始前同步交给 run-local fence；后置副作用失败仍传播原始错误，不能吞错，也不能让后续 failure cleanup 使用旧版本产生必然 CAS miss。初始范围解析、旧 pipeline 查询等准备阶段失败仍必须走同一 fenced terminal failure 语义，不能退回外层无围栏失败投影。command/task raw lookup、pipeline cancel 或后置基础设施错误不能伪装成 ownership lost，必须向 command failure/retry/recovery 传播；只有明确 command/task 不再属于当前执行、任务取消或 AbortSignal aborted 才能结束旧执行。
- Auto-execution 的基础设施失败必须由持有当前 ownership 的 runner 完成最后一次 fenced failure projection，再把原始错误交给 command failure/retry/recovery。通用 background wrapper 只能投影普通导演 runner 的失败；带 `AUTO_EXECUTION_RUN_FAILED` 标记的错误表示 auto-execution 已经完成或尝试过 fenced 投影，外层不得再调用无 ownership 的 `markTaskFailed`。若 retry 在 terminal CAS 前赢得 ownership，旧 runner 禁止清理 pipeline/runtime/seed，但原始基础设施错误仍必须传播，不能把 CAS miss 当成命令成功。
...
- 人工审校调用 Planner 也必须按结构化动作判断：`recommended=true` 只表示存在建议，不能单独触发重规划；`local_patch_plan` 留在章节局部修复范围，只有 `action=stop_for_replan` 才调用 Planner。Planner 调用失败必须沿审校错误契约显式返回，禁止 `.catch(() => null)` 把重规划失败伪装成审校成功。
