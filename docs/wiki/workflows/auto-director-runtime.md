...
- ownership fence 的事实快照至少包含任务 `attemptCount + updatedAt`。最终 workflow `updateMany` 必须同时校验 task id、`lane=auto_director`、允许的活动状态、`cancelRequestedAt=null`、attempt 和 updatedAt；CAS miss 统一转换为 ownership lost。CAS 成功后才允许发 transition notification，并用返回行更新本次 run 的快照。数据库/基础设施读取或 pipeline cancel 异常不能伪装成 ownership lost，必须向 command failure/retry/recovery 传播；只有明确不存在、已取消、已请求取消或 AbortSignal aborted 才能结束旧执行。
