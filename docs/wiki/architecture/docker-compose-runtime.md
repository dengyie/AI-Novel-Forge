...
- Compose 解析期配置，例如 `AI_NOVEL_WEB_PORT`、`POSTGRES_DB`、`POSTGRES_USER`、`POSTGRES_PASSWORD`。
- API 容器运行时配置，例如模型供应商 Key、RAG 参数、API body limit。

`services.api.env_file` 只能把变量注入 API 容器，不能参与 Compose 解析期的 `${VAR}` 插值。因此用户复制并修改 `.env.docker` 后，应使用：

```bash
docker compose --env-file .env.docker up -d --build
```

停止同一套配置时也应保持同一份 env-file：

```bash
docker compose --env-file .env.docker down
```

如果用户没有复制 `.env.docker`，可以继续使用 `docker compose up -d --build`，Compose 文件中的默认值会生效。

### 构建上下文边界

仓库根目录是 API 和 Web 镜像的 build context，因此必须维护 `.dockerignore`。它至少应排除：

- `.git/`、本地日志、临时目录和 AI 工具产物。
- 根目录和子包的 `node_modules/`、`dist/`、build 产物。
- `.env`、`.env.local`、`.env.docker`、`server/.env`、`client/.env` 等密钥文件。
- SQLite 数据库、server storage、临时数据库和桌面打包产物。

`.env.docker.example` 和 `.env.example` 必须保留在 build context 外的仓库内容中，供用户复制配置；不要把真实 `.env.docker` 纳入镜像上下文。
...
worker 级 warning 默认应可见，但需要按消息节流。详细任务轨迹继续由 `RAG_VERBOSE_LOG` 控制；基础设施异常不能完全依赖 verbose 开关，否则数据库或 Qdrant 长时间不可达时，用户只会看到知识库任务停滞而没有诊断线索。

### 流式响应代理

Docker Web 入口反代 `/api/` 时必须支持 SSE 和长连接流式响应。后端章节生成、Creative Hub 和部分 LLM 调用会通过 `text/event-stream` 连续写入前端；nginx 默认缓冲会让前端看不到实时进度。

`infra/nginx/ai-novel-web.docker.conf` 的 `/api/` location 应关闭代理缓冲并保留长超时：
