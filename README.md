...
- 手机端打开自动导演、短剧项目、漫画工作台和有声书工作台时，会使用对应页面的移动布局与标题，创作入口不会再回退成宽屏排版或笼统的“更多功能”。
- 手机首页的创作状态指标会保持紧凑的四列布局，数字和卡片间距更适合窄屏查看。
- 角色音色设置与有声书就绪表的操作说明更清晰，区分生成试听、播放试听和参考轨，便于确认角色是否可以进入有声书制作。
- 章节流水线在后台执行者切换期间收到取消时，会完成取消收尾并释放任务租约，任务不会长期停留在“取消中”，后续可以正常重试。
