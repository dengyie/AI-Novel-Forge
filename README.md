...
- 自动写章、审校、修文和状态回写期间如果正文被手动修改，系统会保留最新内容，不会被较早启动的生成结果覆盖，也不会附着旧评分或旧完成状态。
- 章节重写后，后续写作只读取当前正文对应的自动事实，同时保留手动补充的事实。
- 章节正文更新后，后续写作不会再混入旧正文抽取的自动时间线事件、钩子或时间锚点；手动维护的时间线内容仍会保留。
- 章节改稿后，较早启动的摘要、状态、角色动态、资源风险和伏笔整理会停止写入，后续检索只使用当前章节完成同步后的内容。
- 任务恢复不会抢占仍有有效执行者的取消任务，卷级复检也只会收口它实际审过的正文版本。
- 在任务恢复或更换后台执行者的同时发出取消，取消请求仍会作用于当前任务，不会因为执行者切换而继续运行。
- 从任务链接打开已取消的自动导演流程时，任务详情会保持可见，方便查看取消位置和已有结果。
