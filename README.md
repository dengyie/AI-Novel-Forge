...
- 后台章节修文会保护你生成期间的手动编辑，不会让较早启动的修文结果覆盖新正文。
- 自动成书会记录可继续的局部质量问题并推进后续章节，只在明确需要重规划或无法安全继续时暂停。
- 章节任务的取消、超时、重试和恢复状态更加稳定；任务恰好在取消时完成，也会保留真实完成结果，不会误取消关联的自动导演任务。失败、阻塞或等待恢复的任务仍可从任务入口查看和处理。
- 新部署会完整建立章节事实与任务恢复所需的数据结构，避免连续性上下文和恢复能力因缺少结构而降级。
