...
- 自动导演在服务重启或后台执行中断后恢复可继续任务时，会让恢复命令与任务状态保持一致；如果取消先发生，恢复流程不会把任务重新启动。
- 自动导演执行权在后台发生切换时，旧执行者会停止提交完成或失败结果；取消、重试和新执行者接管会保留真实终态，减少任务反复跳转或重复执行。
- 上游模型出现短时故障时，系统会根据本次实际失败的模型选择已配置的可用替代模型，并记住本轮已经失败的目标，避免在同一批模型间反复切换和重复消耗。
- 当跟进提醒存储暂时不可用或处于升级过渡期时，未读数与已读状态会安全降级并留下告警，不会因此中断自动导演的整本生产链。
