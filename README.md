...
- 服务意外重启时会按顺序接管后台任务；恢复完成前，普通创作请求会收到明确的稍后重试提示，避免新请求与恢复工作同时挤占内存。
- 整本章节流水线、自动导演和全书音频封装默认采用保守的单任务执行，降低连续重启和间歇性不可用风险。
- 有声书章节重做、续生成和后台封装会识别当前任务代次；较早的进程不会覆盖新音频，封装结果暂时写入失败时也会继续补写状态。
- 请求内容不是合法 JSON 时会明确返回参数格式错误，不再显示为服务器内部故障。
