...
- 形象演变扫描会额外提取短外貌词条，先放入待确认区，不会自动改写角色档案。
- 角色详情页可以逐条勾选、忽略外貌词条，并把选中的可信词条融合到角色外貌中。
- 外貌融合会同步刷新角色档案外貌和形象演变稳定特征，后续角色图和章节形象图会默认使用融合后的外貌信息。
- 章节形象图生成会优先引用已生成的角色基础形象图；有多张基础形象图时，可以选择本次要用的参考图，生图确认弹窗会显示对应缩略图。
- 外貌维度的原文召回改为专用视觉检索词，减少台词、心理等非外貌内容干扰。
