...
- 知识库设置保存后会立即应用到后台索引与检索，切换 Qdrant、Embedding、分块或并发参数时不会继续使用旧配置；保存期间正在处理的任务会先安全收口，再按需继续。
- 知识库索引和检索在 worker 被系统终止、重启或暂时失联时会自动恢复任务；正在处理的索引不会长期卡住，长任务也不会被误判为空闲。
- 关闭知识库功能时会安全停止后台处理，重新启用后按需继续，减少服务重启和资源紧张时的状态残留。
- 全量重建知识库时，即使当前没有小说，也会继续处理世界设定和知识文档；大量内容会分批进入队列，避免一次性提交过多任务。
- 后台 worker 在冷启动、异常退出、关闭和空闲退出时会保留请求与任务状态，Director 空闲进程也会主动释放通信资源，减少任务卡住和后台内存残留。
- 有声书 m4b 封装进程异常退出后会先收回任务再启动替代进程，避免任务永久停在处理中；服务没有封装任务时也能快速完成退出。
