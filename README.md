...
- 服务恢复期间取消的章节流水线会在后台正确收口，任务不会卡在“取消中”，之后可以直接重试。
- Web 和桌面启动会等后台恢复完成后再进入工作台，减少恢复窗口中的请求失败和状态错乱。
- 有声书续生成结束后，父任务会根据实际可播放章节收口；遇到并发更新、取消或服务恢复时，也不会被旧回调错误改写或长期卡在处理中。
- 有声书失败章节的重试记录会绑定当前生成代次，较早的子任务不会把新一轮生成误标为失败，也不会覆盖新音频。
