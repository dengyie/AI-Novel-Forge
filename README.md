...
- 从文本提取写法时，原文样本会和写法资产一起保存下来，后续回看、比对和继续微调都更顺手。
- 文本里提取到的写法特征不再只停留在一段分析说明里，而是会沉淀成可见的特征池，进入写法编辑后可以像绑定反 AI 规则一样逐项启用或停用。
- 如果某次文本提取没有产出可用特征，编辑页现在会直接提示原因，并提供“重新提取特征”入口，不会再出现看不到选项却不知道问题在哪的情况。
- 调整提取特征的启用状态后，写法规则会同步重编译，后续试写、绑定和复用时更容易看到风格变化。

### 2026-03-22

- 小说创建页新增了“AI 自动导演创建”试验入口，可以先让 AI 给出 2 套整本方向候选，再由你确认或补充修正意见继续生成，减少一上来就要手填大量开书信息的负担。
- 自动导演候选不再是一锤定音的单次输出，如果第一批方案都不满意，现在可以直接告诉系统“哪里不对、想往什么方向改”，继续生成下一批备选，而不用整页重来。
- 整本批量生成和单章运行时链路开始真正收拢：批量 pipeline 现在会走和单章运行时更接近的上下文装配、审阅和修正主链，整本跑出来的结果和单章逐个生成的差异进一步缩小。
- 模型连通失败时的报错信息也更明确了。像上游模型服务断连、连接被重置这类问题，不再只显示笼统的 `Connection error`，而是会直接告诉你是哪个上游服务连不上，更方便排查。
- 小说编辑页补上了“正文开写前的写法确认”环节：可以先让 AI 根据题材、叙事视角、节奏和当前规划，从现有写法资产里推荐 2-3 套候选，再一键设为整本默认写法，降低新手在正式开写前选风格的门槛。
- 章节重规划从“只改当前章”升级成了“按窗口联动重规划”：现在可以带着审计问题一起重算后续多章，运行时面板里也会直接展示本章职责、阶段、必须推进项、必须保留项和这次重规划参考了多少问题，方便追踪整段剧情是怎么被修过的。
- 角色形象图生成后会优先落地到本地文件存储，再通过本地接口稳定回显；角色卡片里可以直接看到本地路径并预览图片，减少外链失效后历史形象图打不开的问题。

### 2026-03-21

- 写法引擎工作区重做为更聚焦的模块化界面：主页面更专注于“选资产、编辑写法、绑定与试写”，反 AI 特征库和内置模板不再长期占据整列，而是按需打开，更适合持续编辑长写法。
- 章节生成和创作中枢对写法约束的接入更深了：写法绑定不再只是摆设，章节生成时能真正带上当前写法，并在生成后补上一轮写法检测与自动修正，减少前后文风突然漂掉的情况。
- 小说创建时的“标题快速选填”弹窗更实用了。现在可以直接在弹窗里补一句标题简报、写参考标题，并看到系统已自动读取到的项目信息，不用来回退出弹窗补字段。
- DeepSeek 模型在标题生成等场景下的 token 上限也做了收口，避免因为超出提供商限制导致标题工坊、候选生成之类的操作直接报错失败。
