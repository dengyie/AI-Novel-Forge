...
- 章节质量反馈现在会进入后续写作和当前章节修复，模型可以直接看到需要补齐的关键节拍、因果承接和具体问题，减少反馈只记录不执行的情况。
- 未通过质量门的章节正文不会再被当作下一章的直接承接依据，但其中的纠偏信息仍会保留给后续写作；修复后的新正文仍需重新通过质量评估，旧反馈不会替代新的审校结果。
- 纠偏反馈块在写作提示预算紧张时不再被无声裁剪，模型每次重写都能看到上一章的失败根因与必须修复项。
- 流式章节生成定稿后会同步刷新质量闭环记录，确保下一次写作和修复读到的是基于最新正文的反馈，而不是过期快照。
- 章节存在明确结束态或不得越过项时，长度债不会再触发越过边界的自动续写；短正文会保留长度债并交给质量门与修复链处理。
