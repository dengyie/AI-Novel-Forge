...
- 自动导演在后台执行权刚好切换或任务重试接管时，会让较早的执行立即退出；新一轮进度、开书设置、任务通知和失败收尾不会再被旧执行误认或覆盖。
- 任务进度已经保存、但通知、状态快照、版本记录或自动放行留痕暂时失败时，后续恢复会沿用已保存的最新进度，减少重复失败和无法正确收尾。
- 待确认状态的自动放行与人工确认或拒绝同时发生时，会保留用户先完成的选择；未实际放行的状态不会写入后续小说状态或自动放行记录。
