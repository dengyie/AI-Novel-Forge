...
- 有声书全书 m4b 封装在服务重启后会自动从上次中断的位置继续，不再因为后台任务与页面任务分离而长期停留在“封装中”。
- 多本有声书同时封装时会按可控并发排队，默认一次处理一本，降低大文件编码对其他写作任务的影响。
- 有声书后台封装遇到音频前置条件缺失或取消等待时，会及时显示明确的 m4b 失败/取消结果，不再长时间停留在“封装中”。
