...
- 自动导演在服务启动恢复任务时会核对最新的取消、重试和完成状态；刚刚取消或重试的任务会保留用户选择，不会被较早的恢复扫描重新排队。
- 同一任务被重复纳入恢复扫描时只会接受一次有效恢复命令，减少重复执行和任务状态来回跳转。
- 取消、重试或后台执行权切换发生在自动成书章节批次中时，旧执行会停止对应章节任务和后续质量处理，不再继续写入完成检查点、质量待优化项或失败状态。
- 新的重试接管后，较早启动的后台执行不会覆盖当前进度；章节任务已经成功或已经取消时，也会保留真实终态，减少任务状态回跳和重复收尾。
