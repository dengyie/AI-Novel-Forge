...
test("端到端：AI 味样本经双轮自审改写，riskScore 递降且套词减少", async () => {
  ...
  // 首轮 72（≥35 触发改写）→ 首轮残留 55（≥50 触发二轮）→ 二轮重检 20（<55 更优，采纳）。
  assert.equal(detection.calls, 3, "首轮检测 + 首轮残留检测 + 二轮质量重检，共 3 次");
  ...
  // residualReport 反映最终交付内容（二轮产物）的真实残留分。
  assert.equal(result.residualReport.riskScore, 20, "residualReport 反映二轮交付内容的真实分");
  // 残留检测分逐轮递降，验证 riskScore 收敛趋势。
  assert.ok(detection.reports[1].riskScore < detection.reports[0].riskScore, "首轮改写后残留低于入口分");
  assert.ok(detection.reports[2].riskScore < detection.reports[1].riskScore, "二轮改写后残留继续下降");
});
...
test("聚类扫描能在样本上捕捉多类 AI 痕迹成簇", () => {
  ...
  assert.equal(applyClusteredRiskFloor(10, clustering.isClustered, true), 45);
  // 成簇但 LLM 判定无 violations（字面量是常用词）→ 不抬分。
  assert.equal(applyClusteredRiskFloor(10, clustering.isClustered, false), 10);
});
