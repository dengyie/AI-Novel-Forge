...
test("assemble tolerates stateChangeProposal.findMany failures from pending-review context loading", async () => {
  // 回归守卫：loadPendingCharacterHardFactReviews 通过 prisma.stateChangeProposal.findMany
  // 查 pending_review 行；它必须 .catch 兜底，否则 DB 抖动会让 GenerationContextAssembler
  // 的 Promise.all reject → assemble() throw → 整章生成中断。失败回退空 Map，
  // prompt 不带"待确认"限定语，等同 confirmed 默认路径。
  const originals = {
    novelFindUnique: prisma.novel.findUnique,
    chapterFindFirst: prisma.chapter.findFirst,
    stateChangeProposalCount: prisma.stateChangeProposal.count,
  ...
  try {
    ...
    prisma.stateChangeProposal.count = async () => 0;
    prisma.stateChangeProposal.findMany = async () => {
      throw new Error("simulated stateChangeProposal DB failure");
    };
    prisma.auditIssue.findMany = async () => [];
    prisma.novelBible.findUnique = async () => null;
    prisma.chapterSummary.findMany = async () => [];
    ...
    assert.ok(assembled, "assemble should not throw when stateChangeProposal.findMany fails");
    assert.ok(assembled.contextPackage, "context package should still be produced");
    assert.ok(
      Array.isArray(assembled.contextPackage.characterHardFacts),
      "characterHardFacts should resolve to a (possibly empty) array, not throw",
    );
  } finally {
    prisma.novel.findUnique = originals.novelFindUnique;
    prisma.chapter.findFirst = originals.chapterFindFirst;
