...
const novelProjectStatusSchema = z.enum(["not_started", "in_progress", "completed", "rework", "blocked"]);

const novelListItemSchema = z.object({
  id: z.string(),
  title: z.string(),
...
const listNovelsInput = z.object({
  query: z.string().trim().min(1).optional(),
  projectStatus: novelProjectStatusSchema.optional(),
  limit: z.number().int().min(1).max(50).optional(),
});

const listNovelsOutput = z.object({
  total: z.number().int(),
  items: z.array(novelListItemSchema),
});

const createNovelInput = z.object({
  title: z.string().trim().min(1),
  description: z.string().trim().optional(),
});

const createNovelOutput = z.object({
  novelId: z.string(),
  title: z.string(),
  status: z.string(),
  chapterCount: z.number().int(),
...
const selectNovelWorkspaceInput = z.object({
...
const selectNovelWorkspaceOutput = z.object({
  novelId: z.string(),
  title: z.string(),
  chapterCount: z.number().int(),
  summary: z.string(),
...
> = {
  list_novels: {
    name: "list_novels",
    title: "列出小说",
    description: "列出当前系统中的小说列表，可按标题筛选。",
    category: "read",
    riskLevel: "low",
  ...
  create_novel: {
    name: "create_novel",
    title: "创建小说",
    description: "创建一本新的小说并返回基础工作区信息。",
    category: "mutate",
    riskLevel: "medium",
    domainAgent: "NovelAgent",
    resourceScopes: ["global", "novel"],
