...
import { normalizeIntentPayload } from "./utils";
...
export async function parseIntentWithLLM(input: PlannerInput): Promise<StructuredIntent> {
  ...
  const parsedJson = await invokeStructuredLlm<Record<string, unknown>>({
    label: `intent:${input.goal}`,
    ...
    schema: z
      ...
      .refine((v) => !Array.isArray(v), { message: "Expected JSON object." }),
    maxRepairAttempts: 1,
  });
