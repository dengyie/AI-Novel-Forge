...
/**
 * 方案 C：质量回环预算账本——共享纯函数层。
 *
 * volume 生成层与 director 编排层都只 import 本模块，杜绝 volume → director 的反向依赖
 * （director → volume 单向成立，volume 反向 import director 会构成循环依赖）。
 * 本模块只依赖 crypto + shared 类型，无副作用，可安全被两端共用。
 */
