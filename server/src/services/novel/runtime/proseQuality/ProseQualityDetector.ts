...
const ENGINEERING_TERM_STRONG_PATTERN = /细纲|情节点|卷纲|功能标签|目标情绪|字数目标|章首钩子|章尾钩子|任务描述|任务单|义务合同|必达项|功能兑付|执行义务|验收检查|must_hit_now|draft_obligation_unmet|scene\s*card|prompt|schema|runtime\s*package|上下文包|系统提示词|修复指令/iu;
const ENGINEERING_TERM_SOFT_PATTERN = /本章|下一章|读者|伏笔|前文|后文|剧情推进|人物弧光|爽点|节奏点|钩子/u;

/**
 * 结构 HUD：扫原文成对【…】（勿依赖 normalizeTextForTermLeakScan 剥壳）。
...
export function detectProseQuality(
  ...
  // 结构 HUD：扫原文【…】；与 term 剥壳正交，词表空也不豁免。
  scanSystemHud(segments, addFinding);
...
/**
 * 原文结构 HUD：成对全角【…】+ 系统面板启发式。
 * - 关键词 / 键值块 → 记 prose_system_hud（默认 high；≥3 块 critical）
 * - 短专名/别称（≤8 字且无关键词/键值）→ 不记
 * - 半角 [] 协议不在本阶段范围（见 hardgate plan §非目标）
 */
function scanSystemHud(
  segments: TextSegment[],
  addFinding: (finding: ProseQualityFinding) => void,
): void {
  type HudHit = {
  ...
  for (const segment of segments) {
    FULLWIDTH_BRACKET_PAIR_PATTERN.lastIndex = 0;
    for (const match of segment.text.matchAll(FULLWIDTH_BRACKET_PAIR_PATTERN)) {
      const inner = (match[1] ?? "").trim();
      if (!inner) {
        continue;
      }
      const index = match.index ?? 0;
      // 对话引号内的角色口癖偶尔可出现【】，仍按结构拦：系统 HUD 不因引号豁免。
      if (!looksLikeSystemHudInner(inner)) {
        continue;
      }
      hits.push({
        segment,
  ...
  const severity: RuntimeAuditIssue["severity"] =
    hits.length >= SYSTEM_HUD_MULTI_BLOCK_CRITICAL_AT ? "critical" : "high";

  for (const hit of hits) {
    addFinding({
      code: "prose_system_hud",
      severity,
      line: hit.segment.line,
      column: hit.column,
      message: hits.length >= SYSTEM_HUD_MULTI_BLOCK_CRITICAL_AT
        ? `正文出现系统/状态面板式 HUD（共 ${hits.length} 处），破坏沉浸感。`
        : "正文出现系统/状态面板式 HUD 结构（全角【】+ 面板特征）。",
      excerpt: hit.excerpt,
      fixSuggestion: "删除伪系统面板/状态栏写法，改成角色可感的动作、对话或环境信息；专名可用书名号或直接叙述。",
    });
  }
}

/** 判断【inner】是否像系统面板而非短专名/别称。 */
export function looksLikeSystemHudInner(inner: string): boolean {
