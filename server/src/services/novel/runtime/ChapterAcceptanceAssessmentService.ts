...
export function normalizeAssessment(
  ...
  // 上游 bf34b514 块3移植（质量优先裁决）：接收闸门 LLM 明确判 patchable_obligation_gap / rewrite_needed 的
  // 情节类软缺口（payoff_touch/goal_change/must_preserve）是「可修复的情节事实」，
  // 不得被确定性代码改判 continue_with_risk 跳过修复——否则 payoff/目标推进缺口只登记不修，
  // 连载里会累积断点。保留本仓 P2-6/offscreen 收窄：character_appearance 类软缺席
  // （offscreen/非必达）不触发修复扰动；failureClassification 仍由 hard/soft 拆分
  // （soft-only 恒 none），故债板/director 不放大，只是多一次 repair_once 局部 patch（若需重写则按 ee979e82 升级）。
  const hasPlotGapRepairWork = (
  ...
  if (status === "accepted" && missingObligations.length > 0) {
    status = hasHardMissingObligation || hasPlotGapRepairWork ? "repairable" : "continue_with_risk";
  }
  ...
  if (
    ...
    && !hasHighRisk
    && (reconciled.repairability === "patchable_obligation_gap" || reconciled.repairability === "rewrite_needed")
    && !hasPlotGapRepairWork
  ) {
