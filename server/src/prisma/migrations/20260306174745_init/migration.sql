...
CREATE TABLE "new_BookAnalysis" (
    ...
    CONSTRAINT "BookAnalysis_documentVersionId_fkey" FOREIGN KEY ("documentVersionId") REFERENCES "KnowledgeDocumentVersion" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
INSERT INTO "new_BookAnalysis" ("createdAt", "documentId", "documentVersionId", "id", "lastError", "lastRunAt", "model", "progress", "provider", "status", "summary", "title", "updatedAt") SELECT "createdAt", "documentId", "documentVersionId", "id", "lastError", "lastRunAt", "model", "progress", "provider", "status", "summary", "title", "updatedAt" FROM "BookAnalysis";
