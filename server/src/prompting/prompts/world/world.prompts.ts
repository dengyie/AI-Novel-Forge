...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "",
      "内容数量要求（可少不可假；信息不足时取下限）：",
      "1. rules.axioms 至少 2 条，优先 3 条；每条尽量含 name/summary，cost/boundary/enforcement 可简写。",
      "2. factions 至少 2 个；forces 至少 2 个；locations 至少 2 个。",
      "3. relations.forceRelations / locationControls 可为空数组，有把握再写 1-3 条。",
      "4. 不要为了凑数量编造与小说主题无关的阵营。",
      "",
      "字段质量要求：",
      "1. title 要像一个可展示的世界名，不要只是小说名。",
      "2. coverSummary 用 1-2 句说明读者一眼能看到的世界面貌。",
      "3. profile.summary 要说明“这个世界长什么样、如何运行、为什么适合这本书”。",
      "4. rules 的 axiom 写清“规则是什么 + 违背会怎样”即可，不必长文。",
      "5. faction 是立场或阶层板块，force 是具体执行者；不要混淆。",
      "6. location 尽量带 narrativeFunction 或 risk；缺字段时可省略。",
      "7. taboo / sharedConsequences 可短列表或空数组。",
      "",
      "面向生成链的要求：",
      ...
      "5. 若某字段暂时写不出，用空数组或省略，不要输出 null 字符串或占位乱码。",
    ].join("\n")),
    new HumanMessage([
