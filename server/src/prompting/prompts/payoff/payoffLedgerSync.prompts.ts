...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "",
      "Book Contract 固定来源（若 major payoffs 文本中出现 refId=book_contract.chapter*Payoff）：",
      "1. 对应阶段回报必须产出 major_payoff 来源，且 sourceRefs.refId 必须原样等于给定 refId（book_contract.chapter3Payoff / chapter10Payoff / chapter30Payoff 之一）。",
      "2. 不要自造其它 book_contract.* refId；不要把固定阶段回报只写成 refLabel 而省略 refId。",
      "3. 若某固定阶段回报已被清空（输入中不再出现对应 refId），不要再为其新建活跃义务。",
      "",
      "状态定义：",
