...
function createSystemPrompt(mode: ChapterTaskSheetQualityPromptInput["mode"]): string {
  ...
  return [
    ...
    "先从标题/目标/任务单推断章型：emotion（情感/关系）、combat（战斗/对抗）、explore（探索/发现）、transition（过渡/转场）。",
    "情感章硬义务应显著少于战斗章；情感章若堆满系统/据点/多线 payoff，判 loadRisk=overloaded。",
    "taskSheet 必须是自然语言作家指令，禁止出现内部质量 code（如 payoff_missing_progress、draft_obligation_unmet、replan_required、prose_*、must_hit_now）。",
    "taskSheet 语义优先检查（规则引擎已先跑，你补 borderline）：",
    ...
    "issues.id 可使用 cognitive_nailing、missing_choice_pressure、missing_scene_anchor 等稳定标识。",
