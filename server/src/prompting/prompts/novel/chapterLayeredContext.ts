  ...
  const blocks: Array<PromptContextBlock | null> = [
    ...
    hasObligationContract
      ? createContextBlock({
        ...
        allowSummary: false,
        content: [
          "本章戏核职责（结果须在情节中成立；禁止照抄本列表措辞）：",
          toListBlock("本章核心结果（须在戏里成立，勿点题复述）", writeContext.obligationContract.mustHitNow, "无"),
          toListBlock("须守住的状态/信息（场面维持，勿说明文）", writeContext.obligationContract.mustPreserve, "无"),
          toListBlock("宜织入的伏笔触感（轻触即可，不必点名）", writeContext.obligationContract.requiredPayoffTouches, "无"),
          toListBlock("须在场上的角色（露脸/行动即可）", writeContext.obligationContract.requiredCharacterAppearances, "无"),
          toListBlock("本章目标变化（用选择与代价呈现）", writeContext.obligationContract.requiredGoalChanges, "无"),
          toListBlock("可延后（不必本章硬写）", writeContext.obligationContract.canDefer, "无"),
          toListBlock("禁止越界（硬）", writeContext.obligationContract.forbiddenCrossings, "无"),
          "以上为结果约束，不是正文提纲；用文学场面完成。",
        ].filter(Boolean).join("\n"),
      })
      : null,
