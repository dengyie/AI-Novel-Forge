...
const writerSystemTemplate = [
  ...
  "2. 必须服从 chapter mission、mustAdvance、mustPreserve 与 ending hook：用场面、动作、对话与信息差完成，禁止把任务单/义务列表逐条转述进正文。",
  "3. obligation contract 中的核心结果、宜触伏笔、须在场角色与目标变化是结果约束：读者应能通过事件与角色反应感知其已发生；意思到位即可，不要求与合同相同措辞。",
  ...
  "",
  "【输出前自查】",
  "在生成正文前，先内部确认：结尾钩子是否成立、本章结果约束是否已在场景中成立、人物硬事实是否违背。确认通过后再开始输出，不需要在正文中输出核查结果。",
].join("\n");
